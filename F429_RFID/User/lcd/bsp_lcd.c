...
#if  LCD_RGB_888
/****************RGB888驅動*****************************RGB888驅動*****************************RGB888驅動**********************************RGB888驅動**********************************RGB888驅動*********************************/
...
/**
  ...
  */ 
/*實際測試可用的較快的配置*/
//#define HBP  24		//HSYNC後的無效圖元
//#define VBP   3		//VSYNC後的無效行數

//#define HSW   1		//HSYNC寬度
//#define VSW   1		//VSYNC寬度

//#define HFP  10		//HSYNC前的無效圖元
//#define VFP   4		//VSYNC前的無效行數

/*根據液晶資料手冊的參數配置*/
#if 0
#define HBP  46		//HSYNC後的無效圖元
#define VBP  23		//VSYNC後的無效行數

#define HSW   1		//HSYNC寬度
#define VSW   1		//VSYNC寬度

#define HFP  20		//HSYNC前的無效圖元
#define VFP   22		//VSYNC前的無效行數
#endif
...
{ 
  ...
  LCD_GPIO_Config();   
	...
	/* 初始化SDRAM，以便使用SDRAM作顯存 */
  SDRAM_Init();
	
	/* 配置 PLLSAI 分頻器，它的輸出作為圖元同步時脈CLK*/
  /* PLLSAI_VCO 輸入時脈 = HSE_VALUE/PLL_M = 1 Mhz */
  /* PLLSAI_VCO 輸出時脈 = PLLSAI_VCO輸入 * PLLSAI_N = 416 Mhz */
  /* PLLLCDCLK = PLLSAI_VCO 輸出/PLLSAI_R = 420/6  Mhz */
  /* LTDC 時脈頻率 = PLLLCDCLK / DIV = 420/6/8 = 8.75 Mhz */
	/* LTDC時脈太高會導花屏，若對刷屏速度要求不高，降低時脈頻率可減少花屏現象*/
	/* 以下函數三個參數分別為：PLLSAIN,PLLSAIQ,PLLSAIR，其中PLLSAIQ與LTDC無關*/
...
void LCD_LayerInit(void)
{
  ...
  /* 層視窗配置 */
  /* 配置本層的視窗邊界，注意這些參數是包含HBP HSW VBP VSW的 */    
	//一行的第一個起始圖元，該成員值應用為 (LTDC_InitStruct.LTDC_AccumulatedHBP+1)的值
	LTDC_Layer_InitStruct.LTDC_HorizontalStart = HBP + HSW;
	//一行的最後一個圖元，該成員值應用為 (LTDC_InitStruct.LTDC_AccumulatedActiveW)的值
	LTDC_Layer_InitStruct.LTDC_HorizontalStop = HSW+HBP+LCD_PIXEL_WIDTH-1;
	//一列的第一個起始圖元，該成員值應用為 (LTDC_InitStruct.LTDC_AccumulatedVBP+1)的值
	LTDC_Layer_InitStruct.LTDC_VerticalStart =  VBP + VSW;
	//一列的最後一個圖元，該成員值應用為 (LTDC_InitStruct.LTDC_AccumulatedActiveH)的值
  ...
  /* 配置混合因數 CA表示使用恒定Alpha值，PAxCA表示使用圖元Alpha x 恒定Alpha值 */       
  LTDC_Layer_InitStruct.LTDC_BlendingFactor_1 = LTDC_BlendingFactor1_CA;    
  LTDC_Layer_InitStruct.LTDC_BlendingFactor_2 = LTDC_BlendingFactor2_PAxCA;
  
  /* 該成員應寫入(一行圖元資料佔用的位元組數+3)
  Line Lenth = 行有效圖元個數 x 每個圖元的位元組數 + 3 
  行有效圖元個數 = LCD_PIXEL_WIDTH 
  每個圖元的位元組數 = 2（RGB565/RGB1555）/ 3 (RGB888)/ 4（ARGB8888） 
  */
  LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((LCD_PIXEL_WIDTH * 3) + 3);
  /* 從某行的起始位置到下一行起始位置處的圖元增量
  Pitch = 行有效圖元個數 x 每個圖元的位元組數 */ 
  ...
  /*配置第 2 層，若沒有重寫某個成員的值，則該成員使用跟第1層一樣的配置 */
  /* 配置本層的顯存首位址，這裡配置它緊挨在第1層的後面*/     
  LTDC_Layer_InitStruct.LTDC_CFBStartAdress = LCD_FRAME_BUFFER + BUFFER_OFFSET;

	/* 配置混合因數，使用圖元Alpha參與混合 */       
...
/**
  * @brief  選擇要控制的層.
  * @param  Layerx: 選擇要操作前景層(第2層)還是背景層(第1層)
  * @retval None
  */
...
/**
  * @brief  設置字體的顏色及字體的背景顏色
  * @param  TextColor: 字體顏色
  * @param  BackColor: 字體的背景顏色
  * @retval None
  */
...
/**
  * @brief 獲取當前設置的字體顏色和字體的背景顏色
  * @param  TextColor: 指向字體顏色的指標
  * @param  BackColor: 指向字體背景顏色的指標
  * @retval None
  */
...
/**
  ...
  * @param Ypos: 直線起點的y座標
  * @param Length: 直線的長度
  * @param Direction: 直線的方向，可輸入LCD_DIR_HORIZONTAL(水準方向) LCD_DIR_VERTICAL(垂直方向).
  * @retval None
  */
...
#else	
/****************RG565驅動***************************RGB565驅動**********************************RGB565驅動*****************************RGB565驅動*****************************RGB565驅動*****************************/
...
void LCD_Clear(uint16_t Color)
{
 ...
 DMA2D_InitStruct.DMA2D_OutputBlue = Blue_Value;
 DMA2D_InitStruct.DMA2D_OutputRed = Red_Value;
 DMA2D_InitStruct.DMA2D_OutputAlpha = (Color&0x8000) ? 0xFF:0x00;		//設置透明度
 DMA2D_InitStruct.DMA2D_OutputMemoryAdd = CurrentFrameBuffer;
 DMA2D_InitStruct.DMA2D_OutputOffset = 0;
 DMA2D_InitStruct.DMA2D_NumberOfLine = LCD_PIXEL_HEIGHT;
 DMA2D_InitStruct.DMA2D_PixelPerLine = LCD_PIXEL_WIDTH;
